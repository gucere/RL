...
class TD3(object):
    ...
    def __init__(self, observation_space, action_space, **userconfig):
        ...
        # 2 instances for Q Networks
        self.Q1 = QFunction(observation_dim=self._obs_dim, action_dim=self._action_n,
                            hidden_sizes=self._config["hidden_sizes_critic"],
                            learning_rate=self._config["learning_rate_critic"])
        self.Q2 = QFunction(observation_dim=self._obs_dim, action_dim=self._action_n,
                            hidden_sizes=self._config["hidden_sizes_critic"],
                            learning_rate=self._config["learning_rate_critic"])

        self.Q1_target = QFunction(observation_dim=self._obs_dim, action_dim=self._action_n,
                                   hidden_sizes=self._config["hidden_sizes_critic"],
                                   learning_rate=0)
        self.Q2_target = QFunction(observation_dim=self._obs_dim, action_dim=self._action_n,
                                   hidden_sizes=self._config["hidden_sizes_critic"],
                                   learning_rate=0)
    ...
    def train(self, iter_fit=32):
        ...
        for i in range(iter_fit):
            ...
            with torch.no_grad():
                ...
                target_Q1 = self.Q1_target.Q_value(s_prime, next_actions)
                target_Q2 = self.Q2_target.Q_value(s_prime, next_actions)
            ...
            loss_Q1 = self.Q1.fit(s, a, target_Q)
            loss_Q2 = self.Q2.fit(s, a, target_Q)
            fit_loss = loss_Q1 + loss_Q2

            actor_loss = torch.tensor(0.0)
            if self.train_iter % self.policy_freq == 0:
                self.optimizer.zero_grad()
                q1 = self.Q1.Q_value(s, self.policy.forward(s))
