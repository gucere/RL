...
class TD3(object):
    ...
    def __init__(self, observation_space, action_space, **userconfig):
        ...
        self.action_space_low = torch.tensor(self._action_space.low, dtype=torch.float32, device=device)
        self.action_space_high = torch.tensor(self._action_space.high, dtype=torch.float32, device=device)
        ...
        self._config = {
            ...
            "buffer_size": int(1e6),
            "batch_size": 128,
            "learning_rate_actor": 2e-4, #0.0005,
            "learning_rate_critic": 2e-4, #0.0005,
    ...
    def train(self, iter_fit=32):
        ...
        for i in range(iter_fit):
            ...
            with torch.no_grad():
                ...
                a = self.policy_target.forward(s_prime)  # Make sure a represents the action output
                gaussian_noise = (torch.randn_like(a) * self.policy_noise).clamp(-self.noise_clip, self.noise_clip)
                ...
                #next_actions = (self.policy_target.forward(s_prime) + gaussian_noise).clamp(self.action_space_low, self.action_space_high)


                target_Q1, target_Q2 = self.Q_target.forward(s_prime, next_actions)
                target_Q_ = torch.min(target_Q1, target_Q2)
                gamma = self._config['discount']
                target_Q = rew + gamma * (1.0 - done.float()) * target_Q_
            ...
            if self.train_iter % self.policy_freq == 0:
                self.optimizer.zero_grad()
                q1, q2 = self.Q.forward(s, self.policy.forward(s))
...
def main(env, player2):
    ...
    for i_episode in range(1, max_episodes+1):
        ...
        while t < max_timesteps:
            ...
            action1 = ddpg.act(ob)
            action2 = player2.act(env.obs_agent_two())
            (ob_new, reward, done, trunc, _info) = env.step(np.hstack([action1,action2]))
            total_reward+= reward
            ddpg.store_transition((ob, action1, reward, ob_new, done))
            ob=ob_new
        ...
        if i_episode % 1000 == 0:
            ...
            for ep in range(100):
                ...
                while True: 
                    _ = env.render()
                    action1 = ddpg.act(obs)
                    action2 = player2.act(env.obs_agent_two())
                    obs, r, d, t, info = env.step(np.hstack([action1,action2]))
                    r_2 = env.get_reward_agent_two(env.get_info_agent_two())
                    total_reward += r
                    total_reward -= r_2
                    rewards.append(total_reward)
                    if d or t: break
